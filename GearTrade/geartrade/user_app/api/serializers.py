...
class RegisterationSerializer(serializers.ModelSerializer):
    password2 = serializers.CharField(write_only=True)                    # required (it must be required)
    name = serializers.CharField(max_length=50, write_only=True)          # this is required now (you can change)
    family_name = serializers.CharField(max_length=50, write_only=True)   # this is required now (you can change)
    phone_number = serializers.CharField(validators=[RegexValidator(regex=r'^09\d{9}$', 
            message='Phone number must be in the format: 09xxxxxxxxx')], write_only=True)     # this is required now (you can change)
    date_of_birth = serializers.DateField(write_only=True)           # this is required now (you can change)
    bio = serializers.CharField(max_length=200, write_only=True, required=False)     # this is required now (you can change)
    ...
    def create(self, validated_data):
        ...
        bio = validated_data.pop('bio', None)                 # if not provided, return None
